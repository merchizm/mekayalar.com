...
<ul class="github-container">
    {#each repos as repo, i}
        {#if i < GITHUB.maximum_repos}
            <RepositoryCard repoName={repo.name} repoDesc={repo.description} repoStars={repo.stargazers_count} repoForks={repo.forks_count} repoLink={repo.html_url} repoMainLanguage={repo.language}/>
        {/if}
    {/each}
    {#if repos.length > GITHUB.maximum_repos}
        <li>
            <a href={`https://github.com/` + SOCIAL_CONNECTIONS.github} class="show_more">Daha fazlasını gör..</a>
        </li>
    {/if}
</ul>
...
<ul class="github-container">
    {#each gists as gist, i}
        {#if i < GITHUB.maximum_gists}
            <GistCard gistDesc={gist.description} gistCreatedAt={gist.created_at} gistCommentsCount={gist.comments} gistLink={gist.html_url} />
        {/if}
    {/each}
    {#if gists.length > GITHUB.maximum_gists}
        <li>
            <a href={`https://gist.github.com/` + SOCIAL_CONNECTIONS.github} class="show_more">Daha fazlasını gör..</a>
        </li>
    {/if}
</ul>
