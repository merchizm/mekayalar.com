<script>
	...
	function check_font() {
		if (fontSizes.indexOf(container.style.fontSize) === 2) n_font.style.display = 'none';
		else n_font.style.display = 'flex';
	}
	...
	function save_offline() {
		if ('serviceWorker' in navigator && navigator.onLine) {
			...
			console.error(
				'seems like your browser does not support offline mode, unfortunately my love.'
			);
		}
	}
	...
	function bionic_reader() {
		...
		for (let i = 1; i < article.length; i++) {
			// iteration starts with 1 because first item includes metadata
			if (allowed_elements.indexOf(article[i].tagName.toLowerCase()) !== -1) {
...
<div>
	<div>
			...
			<label for="mobil_nav" style="z-index: 11">
				<div class="hamburger">
					<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24"
						><path
							d="M3.25 17.625v-1.5h17.5v1.5Zm0-4.875v-1.5h17.5v1.5Zm0-4.875v-1.5h17.5v1.5Z"
						/></svg
					>
				</div>
			</label>
		...
		<button
			class="spotify"
			aria-label={music.is_playing ? music.name + ' - ' + music.artists : spotify}
			on:click={() => {
				window.open(music.is_playing ? music.url : spotify_href);
			}}
		>
